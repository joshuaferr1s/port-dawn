<script>
  ...
  function getEntity() {
    const foundEntity = $entityList.find(el => el.id === id);
    if (!foundEntity) return $goto('/404');
...
{#if !$dataLoading && currentEntity}
  <div class="mt-10 w-11/12 mx-auto pb-5">
    <ActionBar backTo={`/party/${id}`} backText={currentEntity.data.name}>
